...
if ( ! class_exists( 'tn_ads_support' ) ) {
	class tn_ads_support {
		...
		static function render_google_ads( $google_ads_code, $type = 'sidebar_ads' ) {
			...
			$ads_cache = get_transient( 'tn_ads_data_' . $ads_data['data_ad_client'] . '_' . $type );
			...
			if ( empty( $ads_cache ) ) {
				...
				//save to cache
				set_transient( 'tn_ads_data_' . $ads_data['data_ad_client'] . '_' . $type, $str, 6 * 3600 );
